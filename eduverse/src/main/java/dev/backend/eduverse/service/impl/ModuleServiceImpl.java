...
public class ModuleServiceImpl implements ModuleService {
  ...
  public ResponseEntity<ModuleDto> createModule(ModuleDto moduleDto) {
    Course course =
        courseRepository
            .findById(moduleDto.getCourseId())
            .orElseThrow(
                () -> new ResourceNotFoundException("Course", "Id", moduleDto.getCourseId()));
  ...
  public ResponseEntity<List<ModuleDto>> getAllModules() {
    ...
    List<ModuleDto> moduleDtos =
        courseModules.stream()
            .map(courseModule -> modelMapper.map(courseModule, ModuleDto.class))
  ...
  public ResponseEntity<List<ModuleDto>> getByCourseId(Long courseId) {
    ...
    List<ModuleDto> moduleDtos =
        courseModules.stream()
            .map(courseModule -> modelMapper.map(courseModules, ModuleDto.class))
            .collect(Collectors.toList());
    return ResponseEntity.ok(moduleDtos);
  }
}
