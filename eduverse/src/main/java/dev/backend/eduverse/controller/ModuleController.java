...
public class ModuleController {
    ...
    public ResponseEntity<?> getAllModules(@PathVariable int pageNumber){
    	try {
			List<ModuleDto> moduleDtos = moduleService.getAllModulesByPagination(pageNumber, pageSize);
			PageNumberResponse<List<ModuleDto>> response = new PageNumberResponse<List<ModuleDto>>(pageNumber, pageNumber, moduleDtos);
			return ResponseEntity.ok().body(response);
		} catch (Exception e) {
			return ResponseEntity.badRequest().body("Fail to get all modules by Page.");
		}
    }
