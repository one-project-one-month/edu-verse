...
@Tag(
    ...
    description =
        "CRUD REST APIs - Create Pathway, Update Pathway, Get All Pathways, Delete Pathway")
@RestController
...
public class PathwayController {
  ...
  @Autowired private final PathwayService pathwayService;
  ...
  public ResponseEntity<ApiResponse<String>> createPathway(
      @Valid @RequestBody PathwayDTO pathwayDTO) {
    try {
      boolean created = pathwayService.createPathway(pathwayDTO);
      if (created) {
        return ResponseUtil.createSuccessResponse(
            HttpStatus.OK, "Pathway created successfully", "created");
      } else {
      ...
      return ResponseUtil.createErrorResponse(
          HttpStatus.BAD_REQUEST, "Failed to create pathway", errorMessage);
    } catch (DataIntegrityViolationException e) {
      return ResponseUtil.createErrorResponse(
          HttpStatus.BAD_REQUEST, "Failed to create pathway", e.getMessage());
    } catch (Exception e) {
      logger.error("Failed to create pathway", e);
      return ResponseUtil.createErrorResponse(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to create pathway", e.getMessage());
    }
  }
  ...
  public ResponseEntity<ApiResponse<List<PathwayDTO>>> readPathways() {
    try {
      ...
      if (pathwayList.isEmpty()) {
        ...
        return ResponseUtil.createSuccessResponse(
            HttpStatus.OK, "Pathways retrieved successfully", pathwayList);
      }
    } catch (Exception e) {
      logger.error("Failed to retrieve pathways", e);
      return ResponseUtil.createErrorResponse(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to retrieve pathways", null);
    }
  }
  ...
  public ResponseEntity<ApiResponse<String>> updatePathway(
    ...
    try {
      ...
      if (updated) {
        return ResponseUtil.createSuccessResponse(
            HttpStatus.OK, "Pathway updated successfully", "updated");
      } else {
        return ResponseUtil.createErrorResponse(
            HttpStatus.NOT_FOUND, "Pathway not found with ID: " + pathwayId, null);
      }
    } catch (EntityNotFoundException e) {
      return ResponseUtil.createErrorResponse(
          HttpStatus.NOT_FOUND, "Pathway not found with ID: " + pathwayId, e.getMessage());
    } catch (Exception e) {
      return ResponseUtil.createErrorResponse(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to update pathway", e.getMessage());
    }
  }
  ...
  public ResponseEntity<ApiResponse<String>> deletePathway(@PathVariable Long pathwayId) {
    try {
      ...
      if (deleted) {
        return ResponseUtil.createSuccessResponse(
            HttpStatus.OK, "Pathway deleted successfully", "deleted");
      } else {
        return ResponseUtil.createErrorResponse(
            HttpStatus.NOT_FOUND, "Pathway not found with ID: " + pathwayId, null);
      }
      ...
      return ResponseUtil.createErrorResponse(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to delete pathway", e.getMessage());
    }
  }
}
