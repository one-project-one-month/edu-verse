...
public class AnswerController {
    ...
    @GetMapping("")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "All Answers retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "204", description = "No Answers found", content = @Content),
    ...
    public ResponseEntity<?> getAllAnswers() {
        ...
        if (allAnswers.isEmpty()) {
            return ResponseUtil.createSuccessResponse(HttpStatus.NO_CONTENT, "No Answers found", "No answers found");
        }
